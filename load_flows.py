...
def get_data(token, exchange, direction, TA_API_KEY):
    ...
    del data['avg_txn_value']
    del data['avg_txn_value_usd']
...
def get_stablecoin_flows(TA_API_KEY):
    ...
    for sc in list(sc_exch.keys()):
        for exchange in sc_exch[sc]:
            ...
            exch_inflows.rename(columns={'number_of_txns':'number_of_inflow_txns'}, inplace=True)
            exch_outflows = get_data(sc, exchange, 'outflow', TA_API_KEY)
            exch_outflows.rename(columns={'number_of_txns':'number_of_outflow_txns'}, inplace=True)
...
def process_df(stablecoinflows_df):
    stablecoinflows_df['net_flow'] = combined_df['outflow'] - combined_df['inflow']
